...
#define DEBUG_TYPE "alaio_apply"

namespace {
  // AlaioApply - Mutate the apply function as needed 
  struct AlaioApplyPass : public FunctionPass {
    static char ID; 
    AlaioApplyPass() : FunctionPass(ID) {}
...
char AlaioApplyPass::ID = 0;
static RegisterPass<AlaioApplyPass> X("apply_fixup", "Alaio Apply Fixups");

static void registerAlaioApplyPass(const PassManagerBuilder&, legacy::PassManagerBase& PM) { PM.add(new AlaioApplyPass()); }
static RegisterStandardPasses RegisterMyPass(PassManagerBuilder::EP_EarlyAsPossible, registerAlaioApplyPass);
